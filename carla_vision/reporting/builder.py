...
_SUMMARY_ROLE = {
    ...
    "evidence_registry": "evidence_registry_release_manifest",
...
def _extract_metrics(
    ...
    elif source.kind == "evidence_registry":
        for metric, path, unit in (
            ("objects", "source_count", "objects"),
            ("source_manifests", "source_manifest_count", "objects"),
            ("verified_objects", "verified_count", "objects"),
            ("verification_failures", "verification_failed_count", "objects"),
            ("registered_artifacts", "registered_artifact_count", "artifacts"),
            ("failed_source_tree_entries", "failed_source_tree_entry_count", "entries"),
            ("failed_source_tree_files", "failed_source_tree_file_count", "files"),
            ("failed_source_tree_bytes", "failed_source_tree_bytes", "bytes"),
            ("excluded_registries", "excluded_registry_count", "objects"),
        ):
            add(metric, path, unit)
        for metric, path in (
            ("carla_contacted", "generation.carla_contacted"),
            ("simulator_mutated", "generation.simulator_mutated"),
            ("source_objects_modified", "generation.source_objects_modified"),
        ):
            value = _nested(summary, path)
            if isinstance(value, bool):
                _add_metric(
