...
def _checkpoint_options(config: SegmentationConfig) -> tuple[str, dict[str, Any]]:
    ...
    if directory.exists():
        if not directory.is_dir() or not (directory / "config.json").is_file():
            raise ValueError("local SegFormer checkpoint requires a directory with config.json")
        if not any((directory / name).is_file() for name in (
            "model.safetensors", "model.safetensors.index.json"
        )):
            raise ValueError("local SegFormer checkpoint requires model.safetensors")
        return str(directory.resolve()), {"local_files_only": True, "trust_remote_code": False}
    if directory.is_absolute() or checkpoint.startswith((".", "~")):
        raise ValueError("local SegFormer checkpoint directory does not exist")
    revision = options.get("revision")
    if revision is None and checkpoint == DEFAULT_SEGFORMER_B0_CHECKPOINT:
...
def _load_huggingface_runtime(
    ...
    checkpoint, load_options = _checkpoint_options(config)
    try:
        import torch
    ...
    model = SegformerForSemanticSegmentation.from_pretrained(
        checkpoint, use_safetensors=True, **load_options
    )
...
class SegFormerSegmenter:
    ...
    def __init__(self, config: SegmentationConfig) -> None:
        ...
        if requested_revision is None and str(config.checkpoint) == DEFAULT_SEGFORMER_B0_CHECKPOINT:
            requested_revision = DEFAULT_SEGFORMER_B0_REVISION
