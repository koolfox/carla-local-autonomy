"""RGB-only occupancy predictor backed by a TemporalVoxelFlowNet checkpoint."""
...
from ..training.flow_model import TemporalVoxelFlowNet
from ..training.model import TemporalVoxelModelConfig
...
class TemporalFlowPredictor:
    """Expose occupancy/semantics to deployment while keeping flow auxiliary."""

    def __init__(self, config: CameraVoxelModelConfig) -> None:
        if config.checkpoint is None:
            raise ValueError("temporal voxel-flow predictor requires a checkpoint")
        ...
        if not isinstance(payload, dict) or payload.get("model_type") != "TemporalVoxelFlowNet":
            raise ValueError("checkpoint is not a TemporalVoxelFlowNet checkpoint")
        self.spec = _spec_from_checkpoint(payload)
        self.model_config = TemporalVoxelModelConfig.from_dict(dict(payload["model_config"]))
        self.model = TemporalVoxelFlowNet(self.model_config).to(self.device)
    ...
    def predict(self, rgb_history: tuple[np.ndarray, ...], spec: VoxelGridSpec) -> CameraVoxelPrediction:
        if spec != self.spec:
            raise ValueError("runtime voxel-grid spec does not match checkpoint")
        if len(rgb_history) < self.history_frames:
            raise ValueError(f"predictor requires {self.history_frames} RGB frames")
        tensor = torch.stack([self._preprocess(frame) for frame in rgb_history[-self.history_frames :]])
        tensor = tensor.unsqueeze(0).to(self.device)
        with torch.inference_mode():
            outputs = self.model(tensor)
            occupancy = torch.sigmoid(outputs["occupancy_logits"])[0]
            semantics = outputs["semantic_logits"][0]
            flow = outputs["flow_mps"][0]
        return CameraVoxelPrediction(
            ...
            metadata={
                ...
                "rgb_only": True,
                "auxiliary_flow_head": True,
                "predicted_flow_valid_for_planner": False,
                "flow_speed_mean_mps": float(torch.linalg.vector_norm(flow, dim=0).mean().cpu()),
            },
        )


def create_predictor(config: CameraVoxelModelConfig) -> TemporalFlowPredictor:
    return TemporalFlowPredictor(config)
